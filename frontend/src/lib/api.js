...
export const api = {
  ...
  uploadFiles: ({ files, collection = 'default', ...identity }) => {
  ...
  search: ({ query, collection = 'default', limit = 5, ...identity }) =>
    request('/search', {
      query: {
        query,
        collection,
  ...
  listDocuments: ({ collection = 'default', limit = 100, offset = 0, filename, ...identity }) =>
  ...
  listFiles: ({ collection = 'default', ...identity }) =>
    request('/list-files', { query: { collection, ...identityQuery(identity) }, headers: identityHeaders(identity) }),
